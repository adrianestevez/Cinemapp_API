...
bd = mysql.connector.connect(user='adrianestevez', password= '12345678', database='cinemapp')
...
def crear_usuario(correo, contra):
    if existe_usuario(correo):
        ...
        h = h.hexdigest()
        insertar = "INSERT INTO usuario(correo,contrasenna) VALUES(%s,%s)"
...
def iniciar_sesion(correo, contra):
    h = hashlib.new('sha256', bytes(contra,'utf-8'))
    h = h.hexdigest()
    query = "SELECT id FROM usuario WHERE correo = %s AND contrasenna = %s"
...
def insertar_pelicula(pelicula):
    ...
    insertar = "INSERT INTO pelicula \
        (titulo, fecha_visto, imagen, director, anno, usuarioId) \
        VALUES (%s, %s, %s, %s, %s, %s)"
...
def get_peliculas():
    query = "SELECT id, titulo, imagen, fecha_visto, director, anno FROM pelicula"
